...
for i in range(num_sources):
    # wl = const.c / const.f
    # array_size = 5
    sources.append(tx(np.array([0, -0.01]), const.f, 1/num_sources, 0))
...
anim.save('./gifs/single_source_real.gif', writer=writergif)
plt.close()
...
# anim.save('./gifs/single_source_real.gif', writer='imagemagick', fps=20)
writergif = animation.PillowWriter(fps=20)
anim.save('./gifs/single_source_abs.gif', writer=writergif)
plt.close()
